...
public class SNRStepdefs implements Constants {
    ...
    public void dbTablesShouldBeUpdatedSuccessfully() throws SQLException {
        ...
        while (mediametadata.next()) {
            ...
            System.out.println(audio_id_test_source);
        ...
        ResultSet media_speaker_mapping_count = postgresclient.select_query("select count(*) FROM media_speaker_mapping where audio_id in (select audio_id FROM media_metadata_staging where source = 'test_source')");
        ...
        ResultSet media_speaker_mapping_statuscount = postgresclient.select_query("select count(*) FROM media_speaker_mapping where audio_id ='" + audio_id_test_source + "' ");
    ...
    public void correctNumberOfFilesShouldBePresentInTheGoogleBucket() {
        int CatalogueFilecount = gcpConnection.bucketSize(Constants.RAW_CATALOGUED_PATH + audio_id_test_source + "/clean/");
